...
Shader "EasyRoads3D/ER Road Detail"
{
	...
	SubShader
	{
		...
		{
			o.Normal = BlendNormals( UnpackScaleNormal( tex2D( _BumpMap, i.uv_texcoord ), _BumpScale ) , UnpackScaleNormal( tex2D( _DetailNormalMap, i.uv4_texcoord4 ), _DetailNormalMapScale ) );
			float2 uv_MainTex = i.uv_texcoord * _MainTex_ST.xy + _MainTex_ST.zw;
			float4 tex2DNode1 = tex2D( _MainTex, uv_MainTex );
			o.Albedo = ( ( ( tex2DNode1 * _Color ) * ( tex2D( _DetailAlbedoMap, i.uv4_texcoord4 ) * _Color1 ) ) * float4(2,2,2,2) ).rgb;
